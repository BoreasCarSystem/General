...
* `timer` - The time the car should be at the target temperature. Timeformat: HH:MM. If null, the PI should start the A/C immediately. If `enabled` is false, the timer value will be irrelevant.
...
```json
{ "enabled":"<true/false>", "temperature":"<temperature_value>", "timer":"<HH:MM/null>"}
```
